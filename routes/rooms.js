...
// Normaliza nombre de sala: minúsculas, sin espacios
...
router.post('/create', async (req, res) => {
  let { name, password, max_users, session_token } = req.body;
  ...
  try {
    ...
    const roomResult = await pool.query(
      `INSERT INTO rooms (name, password_hash, max_users, created_by_token)
       VALUES ($1, $2, $3, $4) RETURNING id, name, max_users`,
      [name, password_hash, maxUsers, session_token]
    );

    const room = roomResult.rows[0];

    // El creador siempre es member_index = 1
    await pool.query(
...
router.post('/join', async (req, res) => {
  ...
  try {
    ...
    // Si ya es miembro, devolver datos existentes
    ...
    // Asignar siguiente índice disponible
